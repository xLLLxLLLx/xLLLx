...
int head[N*20],dfn[N*20],low[N*20],dp[N*20],d[N*20],nm[N*20],stk[N*20],vis[N*20],col[N*20];
LL s[N],ri[N],all[N*3];
...
void go(int x,int l,int r,int L,int gg){
  if(l==r){ add(x+idx,gg); return ; }
  int mid=(l+r)>>1;
  if(L<=mid) go(lson,l,mid,L,gg);
  else go(rson,mid+1,r,L,gg);
}

void og(int x,int l,int r,int L,int R,int gg){
  if(L<=l&&r<=R){ add(gg,idx+x); return ; }
  int mid=(l+r)>>1;
  if(L<=mid) og(lson,l,mid,L,R,gg);
  if(R>mid) og(rson,mid+1,r,L,R,gg);
}
